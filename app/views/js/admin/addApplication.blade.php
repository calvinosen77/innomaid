      ...
      $(document).ready(function () {
          ...
          form.children("div").steps({
              ...
              onFinishing: function (event, currentIndex) {
                  ...
                  var status = $('#replacement_maid').css('display');
                  if(status == 'block'){
                    $('#new_maid').remove();
                  }else{
                    $('#replacement_maid').remove();
                  }
...
//                  alert("Submitted!");
        ...
        $('input[value="Remove Item"]').click(function(){
            $(this).parent().parent().remove();
            var flag = $('#replacement_maid').css('display');
            if(flag == 'block'){
                var numList = $('[name="replace_list_title"]').length;
                for(var i=0;i<numList;i++){
                    $('[name="replace_list_title"]').eq(i).empty();
                    $('[name="replace_list_title"]').eq(i).append(i+1);
                }
...
//                    $('#ajaxLoadingDiv').css('display','none');
    ...
    function getMaidData(id){
        $.ajax({
            url: "{{ URL::route('admin.maidapplication.getMaidData') }}",
            dataType : "json",
            type : "POST",
            data : {id : id},
            success : function(data) {
                if (data.result == 'success') {
                    var maiditem_data1 = data.maiditem_data;
                    if(maiditem_data1 == null){
                        maiditem_data1 = [];
                    }

                    $('[name=new_nationality]').val(maiditem_data1['nationality']);
                    $('[name=new_passport_no]').val(maiditem_data1['passport_no']);
                    $('[name=new_salary]').val(maiditem_data1['basic_salary']);
        ...
        $.ajax({
            ...
            success : function(data) {
                if (data.result == 'success') {
                    ...
                    $('[name=replacement_nationality]').val(maiditem_data1['nationality']);
                    $('[name=replacement_passport_no]').val(maiditem_data1['passport_no']);
                    $('[name=replacement_salary]').val(maiditem_data1['basic_salary']);
    ...
    function getMaidDataReplacement(id){
        $.ajax({
            url: "{{ URL::route('admin.maidapplication.getMaidData') }}",
            dataType : "json",
            type : "POST",
            ...
            success : function(data) {
                if (data.result == 'success') {
                    ...
                    $('[name=replacement_replace_passport_no]').val(maiditem_data1['passport_no']);
